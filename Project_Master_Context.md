...
## 1. ★ Cursor 행동 제약: 스크립트 제어 허용 & 에디터 조작 금지

### 허용 — C# 스크립트

- 기능 구현에 필요한 **`.cs` 파일 생성(Create)** 및 **기존 코드 수정(Modify)** 은 AI가 직접 판단하여 `Assets/_Project/Scripts/` 등 적절한 경로에 수행해도 됩니다.
...
- [ ] OIIA **2~4P·부스 다패드·4슬롯 동시 셔플** 재검증 (1P·통합 **2026-06-28** 완료)
...
1. [ ] C# 변경만 수행했는가? (에디터 독단 지시 없음)
2. [ ] 에디터 연결 필요 시 §2 가이드를 **채팅에만** 제공했는가? (`01`·`05_*`에 Step-by-Step **미작성**)
3. [ ] `02_개발_진행_일지.md` 에 **확인된 오늘 날짜** 섹션을 추가·갱신했는가?
4. [ ] 영향 문서(`01`, `04`, `05_*`, `03`) 및 **본 파일 §6** To-Do를 갱신했는가? (`05_*`에 검증 체크리스트·에디터 가이드 **미추가**)

---

*마스터 파일 갱신: 2026-07-10 — SNES Prefab 완제품만 정리*
