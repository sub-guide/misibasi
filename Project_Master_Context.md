...
사용자가 요청·승인한 **원래 의도**가 안 되거나 막히면, AI가 스스로 우회책을 만들어 넣지 않습니다.

| 금지 예 | 해야 할 일 |
|---------|------------|
| 본이 없다고 추정 어깨 좌표·폴백 위치 자동 생성 | 멈추고 **사용자에게 알린 뒤** 배치/바인딩/값을 어떻게 할지 질문 |
...
**원칙**: 막히면 **대안 코딩이 아니라 질문**이다. 사용자가 명시한 구현만 한다.
...
| `Documentation/05_Coffin_Dance.md` | 관짝춤(Coffin Dance) 기술 문서 (순수 충돌·자유 점프, 씬 조립 후속) |
