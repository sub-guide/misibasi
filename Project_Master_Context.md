...
**불확실하면 `.cs` 수정 금지** → 질문·채팅 플랜·사용자 승인 후에만 코딩 (§1 ★ 플랜 모드 의무).
...
> ⚠️ **독단 금지 2축**  
> ① **불확실할 때 `.cs`를 추론으로 쓰지 않는다** (아래 ★ 플랜 모드)  
> ② **Unity 에디터를 허락 없이 지시하지 않는다** (아래 ★ 에디터 금지)

### ★ 금지 — 불확실성 시 독단 코딩 (플랜 모드 의무)

아래 **어느 하나라도** 해당하면, AI는 **독단 추론·임의 판단으로 `.cs`를 작성·수정·Apply 해서는 안 됩니다.** 무리한 즉시 개발은 **원천 금지**입니다.
...
**의무 행동 (채팅 플랜 모드 — Cursor UI Plan Mode 전환은 선택)**

1. **질문**: 불확실 요소·기획 맹점·추가 필요 정보를 **명시적으로** 사용자에게 묻는다.
2. **플랜 제안**: `.cs` 작성 **전에** 아키텍처·구현 방향을 **논리적인 단계별 플랜**으로 채팅에 제시한다.
3. **승인 후 코딩**: 사용자와 핑퐁으로 의도·플랜이 **100% 확정**된 뒤에만 코딩·파일 수정에 진입한다.

**승인으로 인정하는 표현 (명시적일 것)**

- 「진행해」「OK」「그 플랜으로」「승인」 등 **플랜 승인**
- 「알아서 해」 등 **명시적 위임** (위임 범위 밖의 새 모호함은 다시 플랜)

**즉시 `.cs` 수정이 허용되는 예외 (모호함 없음)**

- 오타·컴파일 에러·명확한 한 줄 버그 수정
- **이미 승인된 플랜**의 기계적 반영 (플랜 범위를 넘는 새 설계는 다시 플랜)

### 허용 — C# 스크립트 (**의도가 확정된 뒤에만**)

- 기능 구현에 필요한 **`.cs` 파일 생성(Create)** 및 **기존 코드 수정(Modify)** 은, 위 ★ 플랜 모드를 통과했거나 모호함이 없는 경우에 한해 AI가 `Assets/_Project/Scripts/` 등 적절한 경로에 수행해도 됩니다.
...
1. [ ] **불확실하면** 질문·채팅 플랜·**명시적 승인** 없이 `.cs`를 건드리지 않았는가? (§1 ★ 플랜 모드 — 모호함 없는 예외만 즉시 허용)
...
*마스터 파일 갱신: 2026-07-12 — OIIA DjPad Unpressed 명도 100 (공통 미적용)*
